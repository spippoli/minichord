...
export function SimulatorConsole() {
  ...
  const log = (text: string) =>
    setLogs((previous) => [{ at: performance.now() - started.current, text }, ...previous].slice(0, 40))
  ...
  const presetLoad = () => {
    ...
    log(`queued a 254-message preset load in ${(performance.now() - started).toFixed(1)} ms`)
  }

  return (
    <main style={{ fontFamily: 'monospace', padding: '1rem', display: 'grid', gap: '1rem' }}>
      ...
      <div style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => send([0xf0, 0, 0, 2, 3, 0xf7])}>save to bank 3</button>
        <button onClick={() => send([0xf0, 0, 0, 3, 0, 0xf7])}>reset bank 0</button>
        <button onClick={() => sim.pressPresetButton(1)}>preset button up</button>
        <button onClick={() => sim.pressPresetButton(-1)}>preset button down</button>
        <button onClick={() => (connected ? sim.disconnect() : sim.reconnect())}>
